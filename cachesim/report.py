...
def build_report(hierarchy, trace_name=None):
    ...
    lines.append(title)
    lines.append(bar)
    lines.append(f"Total accesses : {h.accesses:>12,}"
                 f"   (reads {h.reads:,} / writes {h.writes:,})")
    lines.append(f"DRAM accesses  : {h.memory_accesses:>12,}"
    ...
    for i, level in enumerate(h.levels):
        c = level.cache
        lines.append(f"--- {c.name}: {_size_str(c.size)}, "
        ...
        lines.append(f"  hits        : {c.hits:>12,}   "
                     f"(reads {c.read_hits:,} / writes {c.write_hits:,})")
        lines.append(f"  misses      : {c.misses:>12,}   "
                     f"(reads {c.read_misses:,} / writes {c.write_misses:,})")
        lines.append(f"  local miss rate  : {_pct(c.misses, c.accesses)}"
                     "   (misses / accesses that reached this level)")
        lines.append(f"  global miss rate : {100 * h.global_miss_rate(i):6.2f}%"
                     "   (misses / all CPU accesses)")
        lines.append(f"  evictions   : {c.evictions:>12,}"
                     f"   (writebacks of dirty blocks: {c.writebacks:,})")
        if c.track_3c and c.misses:
            lines.append("  miss breakdown (the 3 Cs):")
            lines.append(f"    compulsory : {c.compulsory_misses:>12,}  "
                         f"{_pct(c.compulsory_misses, c.misses)} of misses"
                         "  (first-ever touch, unavoidable)")
            lines.append(f"    capacity   : {c.capacity_misses:>12,}  "
                         f"{_pct(c.capacity_misses, c.misses)} of misses"
                         "  (working set bigger than the cache)")
            lines.append(f"    conflict   : {c.conflict_misses:>12,}  "
    ...
    lines.append(f"AMAT (measured)         : {h.measured_amat():8.3f} cycles"
