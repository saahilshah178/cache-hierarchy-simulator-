...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version",
                        version=f"cachesim {__version__}")
    ...
    p_run.add_argument("--config", metavar="FILE",
                       help="JSON hierarchy config (default: built-in "
                            "L1/L2/L3 config, see configs/default.json)")
    p_run.set_defaults(func=_cmd_run)

    p_gen = sub.add_parser("gen-traces", help="write the sample traces")
    p_gen.add_argument("names", nargs="*", choices=sorted(SAMPLE_TRACES),
                       help="which traces to write (default: all)")
    p_gen.add_argument("--out-dir", default="traces",
                       help="output directory (default: traces)")
    p_gen.set_defaults(func=_cmd_gen_traces)

    p_sweep = sub.add_parser(
        "sweep", help="sweep associativity and size; plot miss rates",
        add_help=False)
